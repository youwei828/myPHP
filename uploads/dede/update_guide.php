...
else if($dopost=='getlist')
{
    ...
    else
    {
        ...
        $allFileList .= "<div class='upinfotitle'>补丁安装前请确保已进行全站备份！<br/>二次开发过的文件请谨慎区分，避免被覆盖！<br/><font color='green'>以下是需要下载的更新文件（路径相对于DedeCMS的根目录）：</font></div>\r\n";
        $filelists = explode("\n",$filelist);
...
else if($dopost=='down')
{
    ...
    if(empty($startup))
    {
        ...
        ShowMsg("成功下载并保存文件：{$files[$curfile]}； 继续下载下一个文件。", "update_guide.php?dopost=down&curfile=".($curfile+1), 0, 100);
        exit();
    }
    else
    {
