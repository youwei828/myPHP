...
class Archives
{
    ...
    function ReplaceKeyword($kw,&$body)
    {
        ...
        while ($row = $this->dsql->GetArray()) {
            ...
            $item = "<a href='$key_url' target='_blank'><u>$key</u></a>";
            $keywords[$key]['item'] = $item;
            $keywords[$key]['item_md5'] = md5($item);
        ...
        foreach ($keywords as $key => $value) {
            ...
            if($cfg_replace_num > 0) {
                $body = preg_replace("#{$key}#i", $value['item_md5'], $body, $cfg_replace_num);
            } else {
                $body = str_ireplace($key, $value['item_md5'], $body);
            }
        }
